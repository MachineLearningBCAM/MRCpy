...
In this example, I am extending the Phi parent class according to the needs of the mappings.
You can choose the best feature mapping class for extension according to your requirements.
"""
...
# Custom phi example: Generating the linear kernel modified by multiplying a constant
...
class myPhi(Phi):
	...
	unless you need any extra parameters from the user for your feature mapping.
	In our example here, we don't actually need this 
	as we are not using any extra parameters here but it is defined here as an example.
	...
	def fit(self, X, Y=None):
		...
		# so we multiply by number of classes for totat length
	...
	def transform(self, X):
		...
		#       so the results after using this feature mappings might not be satisfactory
		X_feat = X * 2
	...
	def eval(self, X, Y=None):
		...
		"""
		Computes the complete feature mapping vector corresponding to instance X.
		X can be a matrix in which case the function returns a matrix in which
		...
		"""
		# Here in this example, we want to use the one-hot encoded feature mappings.
