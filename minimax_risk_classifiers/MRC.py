...
class MRC(BaseEstimator, ClassifierMixin, _MRC_):
    ...
    zhi_l_ : array-like of shape (n_features) or float
        Parameter learnt by solving the lower bound optimization of MRC when solved using CVXpy.
    ...
    def _minimaxRisk(self, X):
        ...
        if self.use_cvx:
            ...
            self.mu_, self.zhi, self.nu_ = self.trySolvers(objective, constraints, mu, zhi, nu)

            # Upper bound
            self.upper_= (1/2)*(self.b - self.a).T@self.zhi - (1/2)*(self.b + self.a).T@self.mu_ - self.nu_

        elif not self.use_cvx:
            # Use the subgradient approach for the convex optimization of MRC

            if self.loss == '0-1':
                # Get the constraint matrix M and the corresponding norm of the class subset h
                ...
                # Define the subobjective function and its gradient for the 0-1 loss function.
                f_ = lambda mu : M@mu + h
                g_ = lambda mu, idx : M.transpose()[:, idx]

            elif self.loss == 'log':

                # Define the subobjective function and its gradient for the log loss function.
                f_ = lambda mu : scs.logsumexp((phi@mu), axis=1)
            ...
            if self.warm_start:
                ...
                try:
                    self.upper_params_= self.nesterovOptimization(m, self.upper_params_, f_, g_)
    ...
    def getLowerBound(self):
        ...
        if self.use_cvx:
            ...
            if self.loss == '0-1':
                ...
                # To define the objective function and the gradient for the 0-1 loss function.
                # epsilon
                eps = np.clip(1 + phi@self.mu_ + self.nu_, 0, None)
                c= np.sum(eps, axis=1)
                zeros= np.isclose(c, 0)
                ...
                # Using negative of epsilon for the nesterov accelerated optimization
                eps = eps - 1
                ...
                # To define the objective function and the gradient for the log loss function.
                # Using negative of epsilon for the nesterov accelerated optimization
                eps = phi@self.mu_ - scs.logsumexp(phi@self.mu_, axis=1)[:, np.newaxis]
                eps = eps.reshape((n*r,))

            phi = phi.reshape((n*r, m))
            # Defining the partial objective and its gradient.
            f_ = lambda mu : phi@mu + eps
            g_ = lambda mu, idx : phi.transpose()[:, idx]
            ...
            if self.warm_start:
                # Start from a previous solution.
                try:
                    self.lower_params_= self.nesterovOptimization(m, self.lower_params_, f_, g_)
                except AttributeError:
                    self.lower_params_ = self.nesterovOptimization(m, None, f_, g_)
            else:
                self.lower_params_ = self.nesterovOptimization(m, None, f_, g_)   
            ...
            # Maximize the function as the nesterov optimization gives the minimum
    ...
    def nesterovOptimization(self, m, params_, f_, g_):
        """
        Solution of the MRC convex optimization(minimization) using the Nesterov accelerated approach.
